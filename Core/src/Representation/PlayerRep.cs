...
namespace LabFusion.Representation
{
    public class PlayerRep : IDisposable {
        ...
        private void OnSwapAvatar(bool success) {
            var rm = RigReferences.RigManager;
            MelonLogger.Msg(avatarId);
            if (!success) {
                Core.src.Representation.BarcodeDownloader.TryDownloadBarcode(this, avatarId);
        ...
        private void OnRepLateUpdate() {
            ...
            if (!rm._avatar.IsNOC()) {
                // Swap the avatar
                if (_isAvatarDirty) {
                    Cysharp.Threading.Tasks.UniTask<bool> SwapAvatarTask = rm.SwapAvatarCrate(avatarId, true, (Action<bool>)OnSwapAvatar);
